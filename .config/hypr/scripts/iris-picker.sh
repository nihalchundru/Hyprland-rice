...
# If quickshell bar is active, use QS wallpaper popup
# but first update WallpaperSwitcher to show all wallpapers
...
THUMB_DIR="/tmp/hyde-thumbs-iris"
mkdir -p "$THUMB_DIR"

mapfile -t WALLS < <(find "$WALL_BASE" -type f \( -iname "*.png" -o -iname "*.jpg" \) | sort)
[ ${#WALLS[@]} -eq 0 ] && notify-send "Iris" "No wallpapers found" && exit 1
...
for wall in "${WALLS[@]}"; do
    ...
    if [ "$label" = "$SELECTED" ]; then
        bash ~/.config/hypr/scripts/iris-apply.sh "$wall"
        break
    fi
done
